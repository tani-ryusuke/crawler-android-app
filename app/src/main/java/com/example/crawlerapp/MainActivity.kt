 ...
 * FPS維持のため、フレームスキップとメモリ再利用を最適化した設計。
 */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val options = OrtSession.SessionOptions().apply {
                ...
                try {
                    // 【ハードウェアアクセラレーション】AndroidのNPUを有効化し、負荷を軽減
                    addNnapi()
                } catch (e: Exception) {
...
//            while (true) {
//                try {
//                    val url = URL("http://192.168.4.2/status")
//
//                    // 変更: network.openConnection(url) に書き換え
...
//        }
                        ...
                        if (bitmap != null) {
                            ...
                            if (skipCounter % 8 == 0) {
                                ...
                                if (newBoxes.isNotEmpty()) {
                                    ...
                                    // 【UX向上】短時間の消失には反応しないようにする
                                    lostCounter++
                                    if (lostCounter > 5) lastBoxes = emptyList()
                                }
                            }
    ...
    private fun runYoloInference(mat: Mat): List<FloatArray> {
        ...
        for (i in 0 until 8400) {
            ...
            // 【精度最適化】動的閾値判定。Personのみ判定基準を緩くする
